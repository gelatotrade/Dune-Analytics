-- ============================================================================
-- DASHBOARD WIDGET: Stablecoin Market Share Pie Chart - 21 Chains
-- ============================================================================
-- Dune Widget-Typ: Pie Chart | X=symbol, Y=volume_share_pct
-- Parameter: {{period}}
-- ============================================================================
...
totals AS (
    SELECT SUM(total_volume) AS global_volume, SUM(total_txs) AS global_txs
    FROM volume_per_coin
)

SELECT
    v.symbol, v.total_volume, v.total_txs,
    v.total_volume / NULLIF(g.global_volume, 0) * 100 AS volume_share_pct,
    CAST(v.total_txs AS DOUBLE) / NULLIF(g.global_txs, 0) * 100 AS tx_share_pct
FROM volume_per_coin v CROSS JOIN totals g
ORDER BY v.total_volume DESC
