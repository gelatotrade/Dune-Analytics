-- ============================================================================
-- DASHBOARD WIDGET: Volume Over Time (Area Chart) - 21 Chains
-- ============================================================================
-- Dune: Stacked Area | X=day, Y=daily_volume, Group=group_label
-- Parameter: {{period}}
-- ============================================================================
...
daily_by_coin AS (
    SELECT DATE_TRUNC('day', t.block_time) AS day, s.symbol,
...
daily_by_chain AS (
    SELECT DATE_TRUNC('day', t.block_time) AS day, t.blockchain,
        SUM(CAST(t.amount_raw AS DOUBLE) / POWER(10, s.decimals)) AS daily_volume,
        COUNT(DISTINCT t.tx_hash) AS daily_txs
    FROM tokens.transfers t
    INNER JOIN stablecoins s
...
SELECT day, symbol AS group_label, 'by_stablecoin' AS chart_type, daily_volume, daily_txs,
    AVG(daily_volume) OVER (PARTITION BY symbol ORDER BY day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS volume_7d_ma
FROM daily_by_coin
UNION ALL
SELECT day, blockchain AS group_label, 'by_chain' AS chart_type, daily_volume, daily_txs,
    AVG(daily_volume) OVER (PARTITION BY blockchain ORDER BY day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS volume_7d_ma
FROM daily_by_chain
