-- ============================================================================
-- DASHBOARD WIDGET: Stablecoin Volume Over Time (Area/Line Chart)
-- ============================================================================
-- Dune Widget-Typ: Area Chart (stacked) oder Line Chart
-- X-Achse: day | Y-Achse: daily_volume | Group By: symbol
--
-- Zeigt die Volumen-Entwicklung aller Stablecoins ueber die Zeit.
-- Stacked Area zeigt sowohl Gesamttrend als auch relative Anteile.
--
-- Parameter: {{period}}
-- ============================================================================
...
-- ===== Chart 1: Volume pro Stablecoin ueber Zeit =====
daily_by_coin AS (
    SELECT
        DATE_TRUNC('day', t.block_time) AS day,
        s.symbol,
        SUM(CAST(t.amount_raw AS DOUBLE) / POWER(10, s.decimals)) AS daily_volume,
        COUNT(DISTINCT t.tx_hash) AS daily_txs
...
-- ===== Chart 2: Volume pro Chain ueber Zeit =====
daily_by_chain AS (
    SELECT
        DATE_TRUNC('day', t.block_time) AS day,
        t.blockchain,
    ...
    WHERE t.block_time >= NOW() - INTERVAL '{{period}}'
        AND t.amount_raw > 0
    GROUP BY DATE_TRUNC('day', t.block_time), t.blockchain
)

-- AREA CHART (stacked): Volumen pro Stablecoin
-- Dune: X=day, Y=daily_volume, Group=symbol, Chart Type=Stacked Area
SELECT
    day,
    symbol AS group_label,
    'by_stablecoin' AS chart_type,
    daily_volume,
    daily_txs,
    -- 7-Tage MA
    AVG(daily_volume) OVER (
        PARTITION BY symbol ORDER BY day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW
    ) AS volume_7d_ma
FROM daily_by_coin

UNION ALL

-- AREA CHART (stacked): Volumen pro Chain
-- Dune: X=day, Y=daily_volume, Group=group_label, Chart Type=Stacked Area
SELECT
    day,
    blockchain AS group_label,
    'by_chain' AS chart_type,
    daily_volume,
    daily_txs,
    AVG(daily_volume) OVER (
        PARTITION BY blockchain ORDER BY day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW
    ) AS volume_7d_ma
FROM daily_by_chain

ORDER BY chart_type, day DESC, daily_volume DESC
